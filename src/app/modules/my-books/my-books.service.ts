...
export class MyBooksService {
  ...
  getBooks = () => {
    this.httpClient.get<{ message: string, books: BookProfileDTO[] }>('http://localhost:3000/api/myBooks')
      ...
      .subscribe(books => {
        ...
        console.log('get books => ', this.booksList);
  ...
  addBook = (title: string, description: string, tradingPreferenceList: string, category: string) => {
    ...
    this.httpClient.post<{ message: string, bookId: string }>('http://localhost:3000/api/myBooks', book)
      .subscribe(({ bookId }) => {
        book['id'] = bookId;
  ...
  updateBooks = books => {
    this.httpClient.put('http://localhost:3000/api/myBooks', books)
      .subscribe(data => {
